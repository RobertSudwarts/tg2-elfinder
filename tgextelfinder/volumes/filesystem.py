...
from tgextelfinder.exceptions import ElfinderErrorMessages, NotAnImageError, DirNotFoundError
...
# replaces `from django.conf import settings`
class Settings(object):
    # place any required parameters here so that they can be picked
    # up by ELFINDER_CONNECTOR_OPTION_SETS
    # This object is just interim, cos you'll almost certainly
    # have to have these parameters somewhere in the `plugged` app.
    # these are as per django.conf global settings
    MEDIA_ROOT = ''
    MEDIA_URL = ''
settings = Settings()
...
class ElfinderVolumeLocalFileSystem(ElfinderVolumeDriver):
    ...
    def mount(self, opts):
        ...
        try:
            rootpath = opts['path']
            log.info ("successfully using opts['path']")
        ...
        if not os.path.exists(rootpath):
            try:
                log.info("rootpath doesn't exist: making")
                os.makedirs(rootpath)
            except:
                raise DirNotFoundError

        if not 'URL' in opts or not opts['URL']:
            log.info ("setting self._options['URL'] = MEDIA_URL")
    ...
    def _save_uploaded(self, uploaded_file, dir_, name, **kwargs):
        """
        Save the FieldStorage object and return its new path
        """
        path = self._join_path(dir_, name)
        target = self._fopen(path, 'wb+')

        for chunk in fbuffer(uploaded_file.file):
            target.write(chunk)
