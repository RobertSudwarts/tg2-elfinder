...
try:
    # note this line works albeit the package
    # installed is Pillow==2.4.0
...
from tgextelfinder.utils.archivers import ZipFileArchiver
from tgextelfinder.exceptions import ElfinderErrorMessages, FileNotFoundError, \
                                     DirNotFoundError, PermissionDeniedError, \
                                     NamedError, NotAnImageError

# you need a sensible replacement for cache...
...
class ElfinderVolumeDriver(object):
    ...
    def upload(self, uploaded_file, hash_dst):
        ...
        if not self._name_accepted(uploaded_file.filename):
            raise Exception(ElfinderErrorMessages.ERROR_INVALID_NAME)
        ...
        # temporary statement til we have a 'size()` method
        if self._options['uploadMaxSize'] > 0 and 200 > self._options['uploadMaxSize']:
    ...
    def _get_cached_dir(self, path):
        ...
        try:
            ...
            self.logger.warn("_get_cached_dir / dir_cache: unable to .get(cache_key) [%s]", cache_key)
            dir_cache = None

        try:
            root_cache = cache.get('elfinder::stat::%sroot' % self.id())
        except KeyError:
            self.logger.warn("_get_cached_dir / root_cache: unable to .get(cache_key) [%s]", 'elfinder::stat::%sroot' % self.id())
            root_cache = None
